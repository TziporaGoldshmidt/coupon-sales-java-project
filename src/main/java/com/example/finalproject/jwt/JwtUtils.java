...
public class JwtUtils implements Filter {
	...
	private String SECRET_KEY = "javainuse";
	...
	public String generateToken(Credentionals userDetails, ClientType clientType) {
	...
	private boolean validateToken(String jwt) throws ServletException {
		...
		try {
			...
			newToken = generateToken(new Credentionals(claims.getSubject(), null), ClientType.valueOf(claims.getIssuer()));
			removeToken(jwt);
		} catch (Exception e) {
			return false;
		}
